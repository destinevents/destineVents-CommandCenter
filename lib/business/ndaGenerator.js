function generateNDAContent(client, address, contact, email, purpose, date, brand) {
  ...
  return `
    ...
    <p>This NDA is entered into on <strong>${fmtDate}</strong> between <strong>${APP_SETTINGS.company.name}</strong> ("Disclosing Party") and <strong>${client}</strong> ("Receiving Party").</p>
    <p><strong>1. Purpose.</strong> The parties wish to explore a business relationship regarding <strong>${purpose}</strong>.</p>
    ...
    <p><strong>${client}</strong><br>${contact ? contact + '<br>' : ''}${address || ''}</p>
  `;
}
...
function buildNDAWindowContent(client, address, contact, email, purpose, date, brand) {
  ...
  return `
<html><head><meta charset="utf-8"><title>NDA — ${client}</title>
<style>
...
<p>AND</p>
<p><strong>${client}</strong> (the "Receiving Party"), with address at ${address || '[Address]'}.</p>
<p><strong>1. Purpose.</strong> The parties wish to explore a business relationship regarding <strong>${purpose}</strong> (the "Purpose"). In connection with this Purpose, the Disclosing Party may disclose certain confidential information to the Receiving Party.</p>
...
<div class="sig">
  ...
  <div><strong>${client}</strong><br><br><div class="sig-line">${contact || 'Authorized Representative'}</div></div>
</div>
</body></html>`;
}
