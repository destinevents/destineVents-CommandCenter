...
const AP_CATEGORIES     = ['Office','Software','Marketing','Travel','Events','Utilities','Taxes','Payroll','Freelancers','Suppliers','Miscellaneous'];
const AP_STATUSES       = ['Pending','For Approval','Approved','Paid','Cancelled'];
const AP_STATUS_CLASS: Record<string, string> = {
  'Pending':      'pending',
...
function apRowHTML(b: Bill): string {
...
function billFormHTML(b: Partial<Bill> = {}, isEdit = false) {
  const catOpts  = AP_CATEGORIES.map(c => `<option${c === b.category ? ' selected' : ''}>${escapeHtml(c)}</option>`).join('');
  const ewtOpts  = (APP_SETTINGS.finance.ewtRates as string[]).map(e => `<option${e === (b.ewt ?? '0%') ? ' selected' : ''}>${e}</option>`).join('');
  const projOpts = `<option value="">— No project —</option>` +
    _projects.map(p => `<option value="${p.id}"${b.project_id === p.id ? ' selected' : ''}>${escapeHtml(p.name)}</option>`).join('');
...
export function openAddBill() {
  _editingBillId = null;
  openModal('New Expense', billFormHTML(), saveBill);
}
...
export function openEditBill(id: number) {
  ...
  if (!b) return;
  _editingBillId = id;
  openModal('Edit Expense', billFormHTML(b, true), saveBill);
}
