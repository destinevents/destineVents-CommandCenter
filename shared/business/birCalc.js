...
export function birQuarterIndex(month) { return Math.floor(month / 3); }

// { q, year } of the most recently *completed* calendar quarter as of `today`.
export function birMostRecentCompletedQuarter(today = new Date()) {
...
export function birQuarterLabel(q, year) { return `Q${q + 1} ${year}`; }

// Statutory deadline (ISO yyyy-mm-dd) per form for a quarter/year.
export function bir2551qDeadline(q, year) {
...
export function bir1701qDeadline(q, year) {
...
export function bir1604cDeadline(year) {
  // Annual information return on compensation withheld — Jan 31 of the following year
  return `${year + 1}-01-31`;
}

export function birDaysUntil(deadlineISO, today = new Date()) {
...
export function birFilingStatus(deadlineISO, isFiled, today = new Date()) {
...
export function birDateInQuarter(iso, q, year) {
  if (!iso) return false;
...
export function birGrossReceipts(invoices, q, year) {
  return invoices
...
export function birExpenses(bills, q, year) {
  return bills
...
export function birFilingsFor(filings, form) {
  return filings
